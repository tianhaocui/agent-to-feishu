...
这个副本在原始 `claude-to-im` 的基础上加了一层 Feishu pairing 审批门禁：

- Feishu 事件接入继续使用官方 `@larksuiteoapi/node-sdk` 长连接
- 未批准用户发消息时，不会进入 AI 会话，而是收到 pairing code
- 管理员可以直接在飞书中审批：
  `/pair pending`
  `/pair approve <CODE>`
  `/pair reject <CODE>`
- 也可以在本机审批：
  `npm run pairing -- list pending`
  `npm run pairing -- approve <CODE>`

相关配置项见 `config.env.example` 中的：

- `CTI_FEISHU_PAIRING_ENABLED`
- `CTI_FEISHU_PAIRING_ADMIN_USERS`
- `CTI_FEISHU_PAIRING_AUTO_APPROVE_USERS`

注意：

- 如果你启用了 pairing，通常不要再配置 `CTI_FEISHU_ALLOWED_USERS`，否则用户会被上游适配器提前拦掉，收不到 pairing code。
- 当前实现把 pairing 放在 Feishu adapter 包装层，优先保证改动集中、兼容原有 runtime / permission gateway。
